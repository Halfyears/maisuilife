...
必须输出合法 JSON，严格遵守以下结构（不得添加其他字段）：
{
  ...
  "scripture_text_full": "和合本圣经原文，无论主题还是经文模式，均只取最核心的 3-5 节，不输出整章",
...
【JSON 输出格式】
{
  ...
  "scripture_text_full": "和合本原文，无论何种模式均只取最核心的 3-5 节，不输出整章",
  "theological_breakdown": [
    "【引言 · 简版】一句开场，引出主题张力",
    "【论点一：标题】核心经文 + 一句应用",
    "【论点二：标题】核心经文 + 一句应用",
...
export async function POST(req: NextRequest) {
  ...
  const typeHint  = meeting_type === 'theme'
    ...
    // 经文模式：只取最核心的 3-5 节，绝不输出整章（整章文字太长会导致 JSON 截断）
    : `经文段落：「${input_query}」\n从该段落中选取最核心的 3-5 节和合本原文放入 scripture_text_full，不要输出整章。`
...
${moodContext}`
  ...
  // Llama 对中文约 1.5-2 token/字；scripture_text_full 限 3-5 节后约 200-400 字
  // Free 总预算：所有字段约 1500 字 × 2 token = 3000 token，4500 留足安全余量
  const systemPrompt = tier === 'premium' ? buildSystemPrompt() : buildFreeSystemPrompt()
  const maxTok       = tier === 'premium' ? 8000 : 4500
