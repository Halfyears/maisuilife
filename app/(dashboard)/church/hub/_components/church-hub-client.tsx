...
import { Loader2, Check, X, Plus, Edit2, ChevronDown, ChevronUp, Users, MapPin, Phone } from 'lucide-react'
...
function ActiveCard({
  ...
  const memberCount = (f.fellowship_members as { user_id: string }[]).length
    ...
    <div className="rounded-2xl border border-stone-100 bg-white/90 overflow-hidden shadow-sm">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1">
              <span className="text-xs text-stone-400 flex items-center gap-1">
                <Users className="h-3 w-3" />{memberCount} 位成员
              </span>
              <span className="text-xs text-stone-400">组长：{leaderName}</span>
            </div>
            {f.meeting_address && (
              <p className="text-xs text-stone-400 mt-0.5 flex items-center gap-1">
                <MapPin className="h-3 w-3" />{f.meeting_address}
              </p>
            )}
            {f.leader_contact && (
              <p className="text-xs text-stone-400 mt-0.5 flex items-center gap-1">
                <Phone className="h-3 w-3" />{f.leader_contact}
              </p>
            )}
          </div>
          <button
            onClick={() => setEditing(v => !v)}
            className="flex items-center gap-1 shrink-0 rounded-xl border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs text-stone-500 hover:border-amber-300 hover:text-amber-700 hover:bg-amber-50 transition-colors"
          >
            <Edit2 className="h-3 w-3" />编辑
          </button>
        </div>

        <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-stone-50 px-2.5 py-0.5">
          <span className="text-[10px] font-mono text-stone-400 tracking-widest">邀请码 {f.invite_code}</span>
        </div>
      </div>
      ...
      {editing && (
        <div className="px-5 pb-5 border-t border-stone-50 pt-4 space-y-3">
          ...
          <div>
            <label className="text-xs font-medium text-stone-500 block mb-1">组长联系方式</label>
            <input
              value={contact}
              onChange={e => setContact(e.target.value)}
              className="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-sm text-stone-700 focus:border-amber-400 focus:outline-none focus:ring-1 focus:ring-amber-400"
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              ...
              disabled={busy}
              className="flex-1 py-2 rounded-xl bg-amber-500 text-xs font-bold text-white hover:bg-amber-600 disabled:opacity-40 transition-colors"
            >
  ...
  return (
    <div className="rounded-2xl border border-stone-100 bg-white/90 px-5 py-5 shadow-sm space-y-3">
      ...
      <button
        ...
        className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 text-sm font-black text-white tracking-widest shadow-md shadow-orange-500/20 disabled:opacity-40 transition-all active:scale-[0.99]"
      >
        {busy ? (
    ...
    <div className="space-y-8">
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-bold text-stone-900">已批准团契（{active.length}）</h2>
        </div>
      ...
      <section>
        {!showCreate ? (
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-amber-300 bg-amber-50/60 py-4 text-sm font-bold text-amber-700 hover:bg-amber-50 transition-colors active:scale-[0.99]"
          >
